...
<div class="relative z-10 min-h-screen flex items-start md:items-center justify-center px-4 py-8">
    <div class="w-full max-w-md">
        ...
        <div class="text-center mb-8">
            ...
            <p class="font-jockey text-white text-lg">Find your account ID</p>
        </div>

        <!-- 카드 -->
        <div class="bg-black/50 backdrop-blur-sm rounded-2xl p-8 border border-gray-700">
            <h2 class="font-jockey text-2xl text-white mb-6 text-center">아이디 찾기</h2>
            ...
            <form id="f" class="space-y-6" autocomplete="off">
                ...
                <!-- 아이디 찾기 버튼 -->
                <button id="btnGoFindId" type="button"
            ...
            <span id="findText" class="flex items-center gap-2">
              아이디 찾기
              <i data-lucide="search" class="w-4 h-4"></i>
            </span>
                    <span id="spinnerFind" class="hidden w-4 h-4 border-2 border-black border-t-transparent rounded-full animate-spin"></span>
                </button>
...
<script>
    ...
    // ===== 상태 =====
    let emailAuthNumber = "";   // 서버가 내려주면 비교
    ...
    const btnFindId     = document.getElementById('btnGoFindId');
    ...
    function setBtnLoading(b){ if(b){findText.classList.add('hidden'); spinnerFind.classList.remove('hidden'); btnFindId.setAttribute('disabled','true');}
    else{spinnerFind.classList.add('hidden'); findText.classList.remove('hidden'); btnFindId.removeAttribute('disabled');}}
    ...
    btnConfirm.addEventListener('click', async () => {
        ...
        try {
            ...
            if (json.existsYn === 'Y') {
                ...
                emailAuthNumber = (json.authNumber ?? '').toString(); // 서버가 줄 때만 프론트 비교
            } else {
    ...
    btnVerify.addEventListener('click', () => {
        ...
        if (emailAuthNumber) {
            ...
            // 서버에서 토큰 검증 방식이면 별도 검증 엔드포인트를 쓰도록 변경
    ...
    btnFindId.addEventListener('click', async () => {
        ...
        try {
            const body = new URLSearchParams(); body.append('email', email);
            const res = await fetch('<c:url value="/user/searchUserIdProc"/>', {
                method: 'POST', headers: csrfHeaders(), body
            });
            if (!res.ok) throw new Error('net');
            const rDTO = await res.json();

            if (rDTO.userId && rDTO.userId.length > 0) {
                setOk((rDTO.userName ? (rDTO.userName + ' 회원님의 ') : '') + '아이디는 "' + rDTO.userId + '" 입니다.');
            } else {
